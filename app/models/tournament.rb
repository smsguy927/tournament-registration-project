...
class Tournament < ActiveRecord::Base
  ...
  def buy_in
    tournament_type.buy_in
  end

  def reg_fees
    tournament_type.calc_reg_fee
  end

  def max_reentries
    tournament_type.max_reentries
  end

  def percent_paid
    tournament_type.percent_paid
  end
  ...
  def self.display_types
    TournamentType.all.map do |type|
      <<~HEREDOC
        ID: #{type.id} | #{type.name} | Buy in : #{type.buy_in} | Fees : #{type.calc_reg_fee} | Re-entries: #{type.max_reentries}
